...
fcitx::KeyState calculate_modifiers(fcitx::KeyState state, FcitxKeySym keyval, char down)
{
    ...
    if (mask != fcitx::KeyState::NoState) {
        if (down) state = static_cast<fcitx::KeyState>(static_cast<uint32_t>(state) | static_cast<uint32_t>(mask));
        else state = static_cast<fcitx::KeyState>(static_cast<uint32_t>(state) & ~static_cast<uint32_t>(mask));
    }
